...
* Updates some syntax (e.g. using `after_sign_in_path` instead of `redirect_location`)
* Doesn't redirect to the QR code display page after creation -- assumes that the user will set it up later. Disables the QR code controller entirely -- use the helper to show the QR code on your setup page.
* Moves the QR code URL logic into the model, so that you can pass it down via JSON or whatever.
* Shows an "Authentication failed." message if the token is incorrect rather than redirecting to root.
* Adds the ability to remember a computer by setting a secure cookie. The cookie's expiry date can be changed using `Devise.remember_gauth_for` (default: 30.days).
* Passes the `gauth_tmp` ID using the session rather than a param, for prettier URLs and harder hijacking.
...
* Version 0.3.3 - Updated some of the redirect methods to proper align with Devise 2.1.0. Also tidied up some of the test routines to proper replicate Devise 2.1.0. Also fix security issue where incorrect session key was cleared after sign in.
